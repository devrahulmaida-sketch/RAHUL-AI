...
class RahulUI:
    ...
    def _show_setup_ui(self):
        self.setup_frame = tk.Frame(self.root, bg="#00080d",
                                      highlightbackground=T("PRI"), highlightthickness=2)
        self.setup_frame.place(relx=0.4, rely=0.5, anchor="center")

        tk.Label(self.setup_frame, text="◈  RAHUL  SETUP", fg=T("PRI"), bg="#00080d",
                  font=("Courier",15,"bold")).pack(pady=(22,4))
        tk.Label(self.setup_frame, text=f"{VERSION} — Linux AI Assistant",
                  fg=T("MID"), bg="#00080d", font=("Courier",9)).pack(pady=(0,18))
        tk.Label(self.setup_frame, text="GEMINI API KEY (FREE at aistudio.google.com)",
                  fg=T("DIM"), bg="#00080d", font=("Courier",9)).pack(pady=(0,4))

        self.gemini_entry = tk.Entry(self.setup_frame, width=56, fg=T("TEXT"), bg="#000d12",
                                       insertbackground=T("TEXT"), borderwidth=0,
                                       font=("Courier",10), show="*")
        self.gemini_entry.pack(pady=(0,22))

        tk.Frame(self.setup_frame, bg=T("DIM"), height=1).pack(fill="x", padx=24, pady=(0,18))

        tk.Button(self.setup_frame, text="▸  START RAHUL",
                   command=self._save_api, bg="#000000", fg=T("PRI"),
                   activebackground=T("DIM"), font=("Courier",12,"bold"),
                   borderwidth=0, pady=12, padx=30, cursor="hand2").pack(pady=(0,22))

        tk.Label(self.setup_frame,
                  text="Tip: No microphone needed — type your commands!",
                  fg=T("DIM"), bg="#00080d", font=("Courier",8)).pack(pady=(0,12))
    ...
    def _save_api(self):
        ...
        self.set_state("LISTENING")
        self.write_log("SYS: RAHUL online. Type your command and press Enter!")

    # Public aliases for backward compatibility
    def _show_setup_ui_old(self): pass
